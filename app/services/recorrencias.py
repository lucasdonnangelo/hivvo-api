...
def valor_no_mes(
    ...
    1. Recorrência inativa (soft delete) → None em qualquer mês.
    2. Acha a vigência cujo período contém (mes, ano) — pela invariante de
       não-sobreposição, no máximo uma casa (não pressupõe lista ordenada).
    3. Achou → o valor dessa vigência; nenhuma contém → None.
    """
