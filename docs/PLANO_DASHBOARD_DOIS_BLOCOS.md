...
> `GET /statistics/projection?meses=12` → `{series: [{mes, ano, receitas, a_pagar, saldo}]}`,
> série de N meses de FLUXO com `series[0]` = mês default (o destaque) por construção, meses sem
> fluxo com zeros, `meses` validado 1..60. Aditivo (`/monthly`, `/yearly` e `/default-month`
> intocados). Ver "Série de projeção (Bloco 2)" no SESSAO_ATUAL_API.md. Faltam os batches 2–3
> (frontend). Evolução do modelo de projeção
