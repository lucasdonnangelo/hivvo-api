...
**Onde entra ([recorrencias.py](../app/routers/recorrencias.py)):** `RecorrenciaResponse` ganhou `valor_exibicao: Optional[Decimal]` (o `RecorrenciaDetailResponse` herda — cobre **lista e detalhe** com um campo). O router calcula com o mesmo `h = hoje()` na lista (`list_recorrencias`) e no `_detail`. **Projeção intocada** (o campo é de leitura da gestão, não entra em `_lancamentos_*`).

**Testes (`TestValorExibicao`):** início mês seguinte → `valor_vigente=null` e `valor_exibicao=valor` (lista **e** detalhe); vigente hoje → `valor_exibicao == valor_vigente`; **encerrada só passado → ambos null** (a borda); início futuro distante (out/2026) → valor da 1ª vigência futura; múltiplas vigências com uma vigente hoje → `valor_exibicao == valor_vigente` (a de hoje, não a futura).
