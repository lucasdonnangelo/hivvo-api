...
**Próximo passo imediato:** **frontend do Dashboard em dois blocos (batches 2–3)** — Bloco 1 "Seu mês" (mês corrente fixo, 4 campos via `/statistics/monthly`: RECEITAS, DESPESAS=`consumo.despesas`, A PAGAR=`a_pagar` (novo campo, só crédito não saído), SALDO=`saldo` do topo (caixa fim de mês — a definição "receitas−a pagar" foi SUPERSEDIDA pela leva de 09/07, ver PLANO_DASHBOARD_DOIS_BLOCOS); o toggle morre) e Bloco 2 "Sua projeção" (consome `GET /statistics/projection`, `series[0]` = 1º mês FUTURO com fluxo, em destaque). O backend está completo (`/monthly` com consumo `8ac6db5` e `a_pagar`, `/default-month`, `/projection` revisado). Na sequência (web): **frontend da 3d Leva 2** — status da fatura + "marcar paga" via `PUT /invoices/{cartao_id}/{ano}/{mes}/pagamento` (backend pronto, 10/07). Deploy (Fase 5) e hardening pré-deploy **já feitos** — o app está no ar.
...
## Leva 2 (lente 3d) — PagamentoFatura + status de fatura (10/07/2026)

Design completo e decisões em **`docs/PLANO_3D_PAGAMENTO_FATURA.md`** (criado nesta leva — o
doc não existia; o design foi fechado em sessão). Resumo do que entrou:

- **Entidade `PagamentoFatura`** ([models/pagamento_fatura.py](../app/models/pagamento_fatura.py)):
  confirmação de pagamento POR FATURA (cartão + competência). Ausência = não confirmado;
  `pago=False` = "não paguei"; `pago=True` = paga. Migration `a3d9f4c2b7e1` (vazia, sem
  backfill; testada upgrade+downgrade+upgrade no Postgres dev e aplicada).
- **`PUT /invoices/{cartao_id}/{ano}/{mes}/pagamento`** (`{pago: bool}` →
  `{cartao_id, ano, mes, pago, data_pagamento, status}`): upsert idempotente e reversível.
  Valida (a) fatura EXISTE e (b) JÁ FECHOU; pagamento antecipado permitido; fatura aberta ou
  competência vazia → 422; cartão alheio → 404. `data_pagamento = hoje()` só na transição
  para pago (re-PUT preserva).
- **Regra de fechamento derivada** (`data_fechamento_fatura` em
  [services/faturas.py](../app/services/faturas.py)): fechamento da competência (m, a) =
  `dia_fechamento` (clampado) do mês-base = (m, a) − `mes_offset_vencimento`; sem
  `dia_fechamento` → último dia do mês-base. FECHADA ⇔ hoje > fechamento (no dia do
  fechamento a compra ainda entra → aberta).
- **`status` derivado** (`status_fatura`, nunca materializado): `paga` (registro pago=True) /
  `aberta` (hoje <= fechamento) / `a_vencer` (fechada, não confirmada, venc >= hoje) /
  `atrasada` (venc < hoje). Exposto em `FaturaListItem`, `FaturaDetalhe` e
  `FaturaCartaoItem` (+1 query fixa por endpoint). O usuário nunca marca "atrasada".
- **a_pagar re-fundado** ([services/estatisticas.py](../app/services/estatisticas.py)):
  lançamento de cartão pago ⇔ fatura confirmada (`_faturas_pagas_mes`/`_faturas_pagas_ano`,
  +1 query só quando há lançamento de cartão). Unifica Fontes 1 e 2 e mata a presunção por
  data. Parcela SEM cartão (carnê): presunção por vencimento (decisão do Lucas). Projeção
  integral/realizado/a_vir/anual/consumo byte a byte intactos (guardas).
- **`PUT /installments/{id}`**: `pago`/`data_pagamento` REMOVIDOS do update
  (`extra="forbid"` → 422 explícito); ficam `cancelado` e `data_vencimento`. Leituras
  legadas do `pago` mantidas (ParcelaResponse, `?pago=`, `total_parcelas_pagas`) até o batch
  cross-repo — o web (SummaryPage) ainda lê `!p.pago`.
- **Helper único de composição**: `_cond_parcelas_fatura`/`_cond_avulsas_fatura` compartilhados
  por `get_invoice` + `totais_fatura_por_cartao` + `fatura_existe` (a consistência cruzada,
  antes afirmada só em teste, agora é por construção; o teste virou guarda do refactor).
- **Decisões registradas** (PLANO_3D §5): parcela sem cartão = presunção por vencimento;
  leituras legadas do `pago` intocadas nesta leva; compra retroativa em fatura paga = aceitar
  (status continua `paga`; o usuário desmarca/remarca).

**Testes (399, todos verdes) — o que mudou e POR QUÊ (regra mais correta, nada amaciado):**
`TestAPagar` passou a semear `PagamentoFatura` no lugar de `Parcela.pago`;
`test_avulsa_vencida_*` INVERTEU (presunção morta — vencida sem confirmação CONTA);
`test_mes_passado_*` idem (100→400); guarda antigo virou DOIS
(`test_alternar_parcela_pago_nao_muda_nada` + `test_alternar_pagamento_fatura_so_muda_o_a_pagar`,
espelhados no router); contagem de queries do anual 6→7 (a query de pagamentos);
`test_installments_router` reescrito (write de pago → 422; `cancelado`/`data_vencimento`
seguem). Novos: `TestPagamentoFatura` (10: confirmar/desmarcar/aberta/vazia/antecipado/
idempotência/borda do fechamento/sem dia_fechamento/404 alheio/path 422) e `TestStatusFatura`
(4: status nos 3 endpoints + pago=False ≡ ausência). E2E real (uvicorn + SQLite): 23/23.

**⚠️ Consequências de produto:** a_pagar de meses passados/corrente SOBE no deploy (avulsa
vencida não some mais sozinha — pendência até o usuário confirmar). Zerar o a_pagar agora é
operação de usuário — o frontend da Leva 2 (badge de status + botão "marcar paga" na tela de
Cartões por mês) é o próximo batch web.
...
(paga/atrasada — Leva 2, **FEITA em 10/07/2026**, ver seção "Leva 2" acima), frontend;
endpoints existentes (`list_invoices`/`get_invoice`) intocados à época (a Leva 2 depois
adicionou `status` aos dois e refatorou os filtros para o helper único).
